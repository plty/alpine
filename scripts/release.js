...
if (!version) {
    return console.log(
        "Whoops, you must pass in a version number to this command as the argument"
    );
}
...
setTimeout(() => {
    ask(
        "Have you reviewed, committed, and pushed all the files for this release?",
        () => {
...
function writeNewAlpineVersion() {
    let file =
        __dirname + "/../packages/docs/src/en/essentials/installation.md";
...
async function draftRelease(name, after = () => {}) {
    ...
    output += pulls
        .map((pull) => `* ${pull.title} [#${pull.number}](${pull.html_url})`)
        .join("\n");
...
async function getLastRelease() {
    let { data: releases } = await axios.get(
        "https://api.github.com/repos/alpinejs/alpine/releases"
    );

    let lastRelease = releases.find((release) => {
        return release.target_commitish === "main" && release.draft === false;
    });
...
async function getPullRequestsSince(since) {
    let { data: pulls } = await axios.get(
        "https://api.github.com/repos/alpinejs/alpine/pulls?state=closed&sort=updated&direction=desc&base=main"
    );
...
function tagNewRelease(name, content, after = () => {}) {
    return axios
        .post("https://api.github.com/repos/alpinejs/alpine/releases", {
            name: "v" + name,
            tag_name: "v" + name,
